...
Follow the AWS Glue programming guide for DynamicFrames and transformations."""
    ...
    def generate_skeleton(
        ...
        """
        Generate complete code for MANUAL jobs (with validation warnings).

        Attempts to generate functional code even for complex jobs,
        but flags it clearly for human review.
        ...
        """
        prompt = self._build_manual_job_prompt(prediction, structure)
        ...
        try:
            response = self.client.complete(
                ...
                system=self._system_prompt,
                temperature=0.3,
                max_tokens=8000,  # More tokens for complex jobs
            )

            glue_script = self._extract_code(response.content)

            if not glue_script:
                # Fallback to basic skeleton if extraction fails
                glue_script = self._generate_fallback_skeleton(prediction, response.content)
            ...
            return GeneratedJob(
                ...
                terraform=terraform,
                generator_type="llm_based_manual",
                llm_tokens_used=response.total_tokens,
                warnings=[
                    "⚠️ MANUAL job - Code generated but REQUIRES HUMAN VALIDATION",
                    "⚠️ Complex patterns detected - Test thoroughly before deployment",
                    f"⚠️ Blockers: {', '.join(prediction.automation_blockers[:3])}"
                ],
            )

        except Exception as e:
            logger.error(f"LLM generation failed for MANUAL job: {e}")
            return GeneratedJob(
                job_name=prediction.job_name,
                category=prediction.category,
                success=False,
    ...
    def _build_manual_job_prompt(
        self,
        prediction: MigrationPrediction,
        structure: Dict
    ) -> str:
        """Build prompt for MANUAL job - attempts complete code generation."""
        stages_info = self._format_stages(structure)

        blockers_detail = ""
        for blocker in prediction.automation_blockers:
            if "CDC" in blocker or "Change" in blocker:
                blockers_detail += "\n- For CDC: Use Glue Job Bookmarks + timestamp columns for incremental processing"
            elif "SCD" in blocker:
                blockers_detail += "\n- For SCD: Implement Type 2 slowly changing dimension logic with effective dates"
            elif "custom" in blocker.lower():
                blockers_detail += "\n- For custom logic: Translate to PySpark UDFs or native DataFrame operations"
            elif "stored procedure" in blocker.lower() or "SQL" in blocker:
                blockers_detail += "\n- For stored procedures/complex SQL: Convert to Spark SQL or DataFrame API"

        return f"""Generate a COMPLETE, WORKING AWS Glue job for this complex DataStage migration.

Job Name: {prediction.job_name}
Category: MANUAL (complex job requiring careful implementation)
Risk Level: {prediction.risk_level.value}

Known complexity factors:
{chr(10).join(f"- {b}" for b in prediction.automation_blockers)}

Migration hints for these complexities:{blockers_detail}

DataStage Job Structure:
{stages_info}

REQUIREMENTS - Generate COMPLETE working code:
1. Full AWS Glue Python script with all imports
2. GlueContext and SparkContext initialization
3. Job arguments parsing with getResolvedOptions
4. Complete data flow implementation:
   - Source reading (JDBC/S3/Catalog)
   - ALL transformations (joins, lookups, aggregations, filters)
   - Target writing with error handling
5. Proper error handling with try/except/finally
6. Logging statements for monitoring
7. Job commit at the end

For complex patterns, implement best-practice solutions:
- CDC → Use job bookmarks + incremental filtering
- SCD Type 2 → Track effective_date, end_date, is_current
- Lookups → Use broadcast joins for small dimensions
- Aggregations → Use groupBy with proper null handling

Add detailed comments explaining:
- What each DataStage stage was doing
- How it's implemented in Glue
- Any assumptions or simplifications made

Return ONLY the complete Python code, ready to deploy."""
    ...
    def _generate_fallback_skeleton(
        ...
        """Generate fallback skeleton when code extraction fails."""
        return f'''"""
AWS Glue Job: {prediction.job_name}
Category: MANUAL - Complex migration requiring human review

Risk Level: {prediction.risk_level.value}
Blockers: {', '.join(prediction.automation_blockers)}

LLM attempted generation but code extraction failed.
Below is the LLM response for reference.
"""
...
# =============================================================================
#
# LLM Response (for reference):
# {llm_response[:2000]}...
#
# Identified blockers requiring manual handling:
# {chr(10).join(f"# - {b}" for b in prediction.automation_blockers)}
#
# =============================================================================

try:
    # TODO: Implement source reading
    # source_df = glueContext.create_dynamic_frame.from_catalog(...)

    # TODO: Implement transformations
    # transformed_df = ...

    # TODO: Implement target writing
    # glueContext.write_dynamic_frame.from_options(...)

    raise NotImplementedError(
        "This MANUAL job requires human implementation. "
